		...
		case "general":
            if(TASK_D->get_task(task_id, "leader_out") == 1)
				...
				if(sizeof(tmp)) // has general
                    def_curent_arrange_troop["general"]=({TASK_D->get_task(task_id,"def_leader")})+explode(str,",");
				else
			...
			def_curent_arrange_troop["max_av_soldier"]=WARAI_D->get_max_soldier(def_curent_arrange_troop["general"]);
