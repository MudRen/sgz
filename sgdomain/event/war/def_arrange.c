...
void def_arrange_left(int task_id)
{
    ...
    //open temp war time of the country who attack others
    if(TASK_D->get_task(task_id,"type")== 2)// only set temp wartime in war, not in train
        COUNTRY_D->init_temp_wartime(AREA_D->get_area(TASK_D->get_task(task_id,"att_area"),"nation"));
	...
	foreach(string tt in keys(troop)){
		...
		if( tt == "commando") {
			AREA_D->set_area(p_area,"population",AREA_D->get_area(p_area,"population")+troop[tt]); 
            // commado is people
			map_delete(troop, tt);
		}
    };
	AREA_D->set_area(p_area, "troop", troop);
	//tell_user("huaer",sprintf("在def_arrange.c/def_arrange_left()中,troop=%O\n",troop));
...
////
	...
	//printf("def_arrange.c/put_def_troop()在产生新战队之前的地方p_room=%s\n",p_room); 
