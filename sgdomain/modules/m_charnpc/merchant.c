...
void answer_buy(object who)
{
   ...
   if(!COUNTRY_D->nation_exist(CHAR_D->get_char(p_id,"nation")))
   {
		this_object()->targetted_action
			("一$N对$T冷冷地道：我们只和官家做生意。\n",who);
		return;
   }
   if(p_id==p_custom)
   {
		this_object()->targetted_action
			("众$N对$T陪着笑脸道：$m$R怎么胡涂了，我们上笔生意还没谈完呢。\n",who);
		return;
   }
   if(stringp(p_custom)&&find_user(p_custom)&&present(p_id,environment(this_object())))
   {
		this_object()->targetted_action
			("一$N对$T连忙陪笑脸道：$m$R请息怒，小可这边买卖一完，马上就和$R谈。\n",who);
		return;
   }
   lis=(EV_MERCHANT)->query_m_goods(n_id,"list","");
   if(!sizeof(lis))
...

{
	...
	case "begbuy":
		...
		{	this_body()->simple_action("$N道：为自家办点。。。\n");
			...
			this_body()->targetted_action("$N对$T道：我这可是公干，你快点。\n",this_object());
...
void award(object o)
{
	...
	if(random(2))
	{
		object ob;
		this_object()->targetted_action(
			"一个$N悄声对$T说：多谢$m$R关照，这是一点小意思。\n",o);
		this_object()->targetted_action(
			"%^GREEN%^一个$N偷偷塞给$T一两金子。\n%^RESET%^",o);
		ob=new(M_GOLD);
        ob->set_m_num(1);
        ob->move(o);
	}
...
void my_cancel()
{
	...
	this_body()->simple_action(
		"$N大喊救命。\n$N与"+COUNTRY_D->get_country(n_id,"name")+
		"的关系降低了。\n");
