...
//      贵重的物品(1 gold 以上)有一定的存货限量，
//      存货量在更新或者现实日期转换时会自动补满
//      查看 p_mpStorage[物种] 为非零者，该物种为限量
//
...
#define FIXVAL 2
#define MAX_STORAGE 20  // xiaobai: 满仓存货量
#define VALUE_LIMIT 10000   // xiaobai:价值 1 gold 以上的每天存货限量
...
private string m_id;
private mapping p_mpStorage = ([]);    // xiaobai: 存货数量
private int p_nDay; // xiaobai: 日期
...
private void full_storage()
{
    ...
    nNum = sizeof(strGoodsArr);
    
    if ( nNum > 0 )
    {
        if ( sizeof( keys(p_mpStorage) ) == 0 )
        {
            p_mpStorage = allocate_mapping( nNum );
        }           
    
        foreach( string id in strGoodsArr )
        {
            p_mpStorage[id] = MAX_STORAGE;
        }
    }
        
    p_nDay = nLocalTimeArr[LT_MDAY];
...
//   		   (reusetype ? CHINESE_D->chinese_value(to_int(val*(FIXVAL))):"--------")
		...
		// xiaobai: 加上存货的显示
		(p_mpStorage[g] ? chinese_number( p_mpStorage[g] ) : "很多" ) );
	}
...
//    m_l=bmax(l_n,l_l);
	m_l=CHAR_D->get_char(p_id,"ju");
	m_l++;
    m_l=(m_l+1)/2;
    p_level=OBJ_D->get_obj(ans,"level");
...
//		else
			return "$N对$T摇摇头，道：不行，"+name+"乃高爵位官员所用之物，$R还是升了爵位后再来吧。\n";
	}
...
// xiaobai: 最后一件物品，非卖，主要是要维持这个 mapping 成员的数据为非0
	if ( p_mpStorage[ans] == 1 )
	{
		maker->targetted_action("$N不好意思地对$T道：我这儿只剩下一件陈列品了，不能卖给$R，请明天再来。\n",me);
		return;
	}
//// xiaobai
