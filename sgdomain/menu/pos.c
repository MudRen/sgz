...
void get_adjust_stuff_name(string stf);
void get_adjust_price_name(string stf);
mixed am_i_king();
...
void done_independence(string cfm) {
	int tmp;
	mapping troop1,troop2;//2001.4.13
	...
	CHAR_D->set_char(p_id,"reputation",
		CHAR_D->get_char(p_id,"reputation")/10*11);
        //Added by suicide for clear guoshi info when ind
	if (COUNTRY_D->get_country(p_nation,"gs")==p_id)
	    COUNTRY_D->set_country(p_nation,"gs",0);   
        
        //Added by suicide for in area soldier morale, dec gold for ind
        //Added by suicide for moving NPC army back to the captial of the NPC ,when player ind 2001.4.13
        if (CHAR_D->get_char(p_nation,"type")==TYPE_NPC)  //NPC为君主的地区独立，部队要返回首都
        {
         string m_id;
         m_id = AREA_D->get_area_real_leader(p_area);
         //如果此时的兵力小于系统挑选成为太守时的兵力,则不予独立.
         tmp = AREA_D->get_area_soldier(p_area,"footman") - CHAR_D->get_char(m_id,"npcarmy");
         printf("m_id=%s tmp=%d \n",m_id,tmp);
         if (tmp<0) 
         {
         write("%^RED%^你没有培植个人的部队势力，现在的军队都是忠于国王的，所以还是以后再独立吧！\n");
         return;
         }
         AREA_D->set_area_soldier(p_area,"footman",tmp);
         AREA_D->add_soldier(CHAR_D->get_char(p_nation,"area"),"footman",CHAR_D->get_char(m_id,"npcarmy"),180,180);
        }
        //Added end	
...
void done_move_me(string p_where,string cfm) {
	...
	CHAR_D->set_char(p_nation,"ranklocal",R_SG);
        COUNTRY_D->set_country(p_nation,"captial",p_where);    //2001.4.13
...
/////////////////////////////地区物资最大库存量设定////////////////////
void display_stuff_info()
{mapping st;
 mixed maxstock;
 printf("现在本地有以下物资：\n");
 st = AREA_D->get_area(p_area,"st");
 if (!mapp(st)||!sizeof(st))
   {printf("%%^RED%%^本地没有任何物资!%%^RESET%%^\n");
    return;
   }
 printf("%%^YELLOW%%^%10s%12s%12s%10s%12s%%^RESET%%^\n","物资ID","物质名称","成本价","数量","最大库存");
 foreach(string s in keys(st))
  {
   if (member_array("maxstock",keys(st[s]))==-1) maxstock="无";
      else maxstock=st[s]["maxstock"];
   printf("%%^CYAN%%^%10s%10s%10d两纹银%10d%10O%%^RESET%%^\n",
           s,BASE_D->get_stuff(s,"name"),
           st[s]["price"],st[s]["num"],maxstock);
  }

}

void adjust_stuff_maxstock(string stf,string num)
{int number,oldstock;
 if (!num||(num=="")) return;
 sscanf(num,"%d",number);
 if (number<0) 
    {printf("%%^RED%%^！最大库存值不能设定为负值%%^RESET%%^\n");
      get_input_then_call
     ((: adjust_stuff_maxstock ,stf:),"调整"+BASE_D->get_stuff(stf,"name")+
                              "的本地最大库存值为：");	
     return;
    }

 AREA_D->set_area_st(p_area,stf,"maxstock",number);
 oldstock = AREA_D->get_area_st(p_area,stf,"num");
 if (oldstock>number)
    {
    AREA_D->add_area_st(p_area,stf,number-oldstock,0);
    COUNTRY_D->add_country_st(p_nation,stf,oldstock-number,
                              AREA_D->get_area_st(p_area,stf,"price"));
    printf("%%^YELLOW%%^由于本地该物资过多，上调%d到国库！%%^RESET%%^\n",oldstock-number);
    }

 display_stuff_info(); 
 get_input_then_call
 ((: get_adjust_stuff_name :), "调整哪一个物资的本地最大库存值：");	
...
{mapping st;	
 if (!stf||(stf=="")) return;
 if (!wizardp(this_body())) return;
 if(!BASE_D->stuff_exist(stf))
   {printf("%%^RED%%^没有"+stf+"这种物资。%%^RESET%%^\n");
   	   get_input_then_call
           ((: get_adjust_stuff_name :), "调整哪一个物资的本地最大库存值：");	
           return;
          }
 st = AREA_D->get_area(p_area,"st");
 if (!mapp(st)||!sizeof(st)||(member_array(stf,keys(st))==-1))
   {printf("%%^RED%%^本地没有该物资!%%^RESET%%^\n");
    get_input_then_call
    ((: get_adjust_stuff_name :),"调整哪一个物资的本地最大库存值：");	 
    return;
   }
...
void do_stuff_setting(){//suicide2000.02.26
	mapping st;
	init_me();
	ret=am_i_local_leader();
	if(stringp(ret)) {	show_invalid_check(); return;}
	printf("这个设定用于设置每个城市各种物资自留的最大值，多余的会自动上缴国库。\n");
        display_stuff_info();
	get_input_then_call
        ((: get_adjust_stuff_name :), "调整哪一个物资的本地最大库存值：");	
        return;
...
/////////////////////////////国库物资交易价格设定////////////////////
void display_country_stuff_info()
{mapping st;
 printf("现在国库有以下物资：\n");
 st = COUNTRY_D->get_country(p_nation,"st");
 if (!mapp(st)||!sizeof(st))
   {printf("%%^RED%%^国库没有任何物资!%%^RESET%%^\n");
    return;
   }
 printf("%%^YELLOW%%^%10s%12s%12s%10s%12s%%^RESET%%^\n","物资ID","物质名称","成本价","数量","交易价格");
 foreach(string s in keys(st))
  {
   printf("%%^CYAN%%^%10s%10s%10d两纹银%10d%10d%%^RESET%%^\n",
           s,BASE_D->get_stuff(s,"name"),
           st[s]["cost"],st[s]["num"],st[s]["price"]);
  }

}

void adjust_stuff_price(string stf,string num)
{int number,cost;
 if (!num||(num=="")) return;
 sscanf(num,"%d",number);
 cost = COUNTRY_D->get_country_st(p_nation,stf,"cost");
 if ((number<cost/5)||(number>cost*5)) //定价的范围为成本的1/5-5之间
    {
    printf("%%^RED%%^价格的设定范围应该在成本的1/5到5倍之间！%%^RESET%%^\n");
    get_input_then_call
    ((: adjust_stuff_price ,stf:),"调整"+BASE_D->get_stuff(stf,"name")+
                              "的商品价格为（单位：银）：");	
    }
  
 COUNTRY_D->set_country_st(p_nation,stf,"price",number);
 display_country_stuff_info(); 
 get_input_then_call
 ((: get_adjust_price_name :), "调整哪一个物资的商品交易价格：");	
 return;
...
void set_stuff_price(){//suicide2000.02.26
	mapping st;
	init_me();
        ret=am_i_king();
	if(stringp(ret)) { show_invalid_check(); return;} 

	printf("这个设定用于设置国库各种物资参加商品交易时的价格（默认值为成本价格的两倍）：。\n");
        display_country_stuff_info();
	get_input_then_call
        ((: get_adjust_price_name :), "调整哪一个物资的商品交易价格：");	
        return;
...
///////////////////////////////////战争时间设置//////////////////////////by listen 1/11/2002
void modify_time(int shift, int tick, int i_answer, string one, string two)
{
	int i_one, i_two;
	int t_one, t_two, t_day;
	
	
	i_one = to_int (one);
	i_two = to_int (two);

	//printf("i_one="+i_one+"i_two="+i_two);
	if ( i_one < 1 || i_one >24 || i_two <1 || i_two > 24 || (i_one == i_two))
	{
		ret = "错误的时间输入,数字必须在1-24之间且不能重复";
		show_invalid_check();
		return;
	}
	if ( tick != localtime(time())[LT_WDAY])
	{
		ret = "日期已经改变，请再试一次";
		show_invalid_check();
		return;
	...
	if (t_two < 0)
		t_two +=24;
	t_day = tick + 1 + i_answer;
	if (t_day > 6)
		t_day -=7;
	COUNTRY_D->set_wartime(p_nation,t_day*2,t_one);
	COUNTRY_D->set_wartime(p_nation,t_day*2+1,t_two);
	printf ("%%^H_RED%%^战争时间已经被设置为 " + i_one + ":00-" + i_one + ":59 以及 " + i_two
		+ ":00-" + i_two + ":59。%%^RESET%%^\n");
	printf ("相对的系统当地时间为 " + t_one + ":00-" + t_one + ":59 以及 " + t_two
		+ ":00-" + t_two + ":59。\n");
	return;
}

void get_time_two(int shift, int tick, int answer, string one)
{
	get_input_then_call((: modify_time , shift, tick, answer, one :), "请输入第二时间段(0点请输入24)[1-24]:");
}
	
void get_time_one(int shift, int tick, string answer)
{
	int i_answer;
	if (answer == "q")
		return;
		
	i_answer = to_int (answer);
	if ( i_answer < 1 || i_answer > 3)
	{
		ret = "错误的日期选择,请选择1，2或3";
		show_invalid_check();
		return;
	}
	get_input_then_call((: get_time_two , shift, tick, i_answer :), "请输入第一时间段(0点请输入24)[1-24]:");
}

void show_time(string choice)
{
	int number,i;
	int temp_day,world_time,curren_time;
	int base_time;
	int tick;
	int shift;
	mapping times;
	...
	times = ([]);
	curren_time = time();
	tick = localtime(time())[LT_WDAY];
	base_time = DAY_D->next_modified_day(curren_time);
	number = to_int(choice);
	switch (number)
	{
		case 1: //here
			shift = 0;
			world_time = curren_time;
			head = "本地时间为：";
			break;
		case 2: //beijing
			shift = DAY_D->beijing_time(curren_time)/3600;
			world_time = DAY_D->beijing_time(curren_time)+curren_time;
			base_time = DAY_D->beijing_time(curren_time)+base_time;
			head = "北京时间为：";
			break;
		case 3: // London
			shift = DAY_D->london_time(curren_time)/3600;
			world_time = DAY_D->london_time(curren_time)+curren_time;
			base_time = DAY_D->london_time(curren_time)+base_time;
			head = "伦敦时间为：";
			break;
		case 4: //Sydney
			shift = DAY_D->sydney_time(curren_time)/3600;
			world_time = DAY_D->sydney_time(curren_time)+curren_time;
			base_time = DAY_D->sydney_time(curren_time)+base_time;
			head = "悉尼时间为：";
			break;
		default:
			ret = "不能识别的选择!\n";
	...
	for (i=0; i<3; i++)
	{
		...
		if (temp_day >6)
			temp_day -=7;
		times[i*2] = COUNTRY_D->get_wartime(p_nation,temp_day*2);
		times[i*2] += shift;
		if (times[i*2] > 23)
			times[i*2] -= 24;
		if (times[i*2] <0)
			times[i*2] +=24;
		times[i*2+1] = COUNTRY_D->get_wartime(p_nation,temp_day*2+1);
		times[i*2+1] += shift;
		if (times[i*2+1] > 23)
			times[i*2+1] -= 24;
		if (times[i*2+1] <0)
	...
	printf("现在的" + head + DAY_D->English_time(world_time)+"\n");
	printf("%%^H_RED%%^请注意校对误差!%%^RESET%%^\n");
	printf("请输入您想要修改的时间段：\n");
	printf("1. " + DAY_D->English_time(base_time) + " 至 " 
		+ DAY_D->English_time(base_time+24*3600-60) + "\n");
	printf("\t预先设定值为 %%^H_RED%%^" + times[0] + ":00-" + times[0] + ":59 和 " + times[1] + ":00-" 
		+ times[1] + ":59。%%^RESET%%^\n\n");
		
	printf("2. " + DAY_D->English_time(base_time+24*3600) + " 至 " 
		+ DAY_D->English_time(base_time+2*24*3600-60) + "\n");
	printf("\t预先设定值为 %%^H_RED%%^" + times[2] + ":00-" + times[2] + ":59 和 " + times[3] + ":00-" 
		+ times[3] + ":59。%%^RESET%%^\n\n");
		
	printf("3. " + DAY_D->English_time(base_time+2*24*3600) + " 至 " 
		+ DAY_D->English_time(base_time+3*24*3600-60) + "\n");
	printf("\t预先设定值为 %%^H_RED%%^" + times[4] + ":00-" + times[4] + ":59 和 " + times[5] + ":00-" 
		+ times[5] + ":59。%%^RESET%%^\n");
		
	get_input_then_call((: get_time_one , shift, tick :), "请选择[1-3,or q]:");
}

void choose_time_zone() 
{
	int hon;
	init_me();
	ret=am_i_king();
	...
	printf("请输入您希望时间以以下哪个时区的时间形式表示?\n");
	printf("1。本地时间（太平洋标准时间 PST）。\n");
	printf("2。北京时间 (Beijing )\n");
	printf("3。伦敦时间 (London)\n");
	printf("4。悉尼时间 (Sydney)\n");

	get_input_then_call((: show_time :), "请选择[1-4]:");	
}
...
void create()
{
   ...
   add_menu_item(toplevel, new_menu_item("招募士兵", (:get_input_then_call,
    ...
    add_menu_item(toplevel, new_menu_item("物资调度设定", (: do_stuff_setting :),"0"));
    ...
    add_menu_item(toplevel, new_menu_item("物资价格设定", (: set_stuff_price :),"m"));
    add_menu_item(toplevel, new_menu_item("更改战争时段", (: choose_time_zone :),"n"));
