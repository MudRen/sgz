...
// 查查今天什么日子，看要不要送礼，送礼期一般为10天
int check_date( object who )
{
    string p_id;
	mixed *local;
	int nRet = 0;
	int bSpecial = 0;  // 是不是特别的日子 
    ...
    {
		this_object()->targetted_action("$N对$T笑道："+
            "只有定居的玩家才有礼物，$R还没有定居，快先去定居吧。\n",who);
        return 0;
    }
    ...
    if ( !bSpecial )
    {
        this_object()->targetted_action("$N白了$T一眼，道："+
            "无缘无故的，问什么礼物。\n",who);
