...
//Add by listen for fake ppl usage
void reduce_ppl(string r_id,int num)
{
	...
	if (curnum>=num)
	{
		LISBUG("have enough people");
		set_room(r_id,"pplnum",curnum-num);
		return;
	}
	set_room(r_id,"pplnum",0);
	num-=curnum;
	foreach (string k in ks) {
		LISBUG("loop to find enough ppl");
		curnum=get_room(k,"pplnum");
		if (curnum >= num)
		{
			LISBUG("find enough ppl");
		   set_room(k,"pplnum",curnum-num);
		   return;
		}
		set_room(k,"pplnum",0);
		num-=curnum;
	}
	LISBUG("no enough ppl");
	level--;
	if (level == 0)
	{
        	bs-=({f_id});
		AREA_D->set_area(p_area,"base",bs);
		dis="由于误用歹人，"+AREA_D->get_area(p_area,"name")+"的"+
	      		BASE_D->get_base(f_id,"name")+"倒闭了。";
	 }
	 else
	 {
	 	while(sizeof(ks)>level*2) {
	           string r=ks[random(sizeof(ks))];
	           map_delete(rooms,r);
	        }
		foreach (string r in ks) 
		{
		   rooms[r]["head"]=0;
		   rooms[r]["pplnum"]=0;
		}
...
string stat_me(string r_id) {
	...
	src=c["src"];
	if(sizeof(src)) {
		ret+="目前库存原料：";//modified by suicide 2000.02.20
	...
	ret+="目前总成本："+c["cost"]+" 两纹银  ";//added by suicide 2000.02.20
...
int room_harvest(string r_id) {
	...
	share=work/qwork;
	//SGSYS(sprintf("quan=%d qwork=%d work=%d share=%d",quantity,qwork,work,share)); 
	work-=(share*qwork);
	...
	AREA_D->add_area_st(p_area,pdt,share,rooms[r_id]["cost"]/share);
	BASE_D->cal_avg_price(pdt,share,rooms[r_id]["cost"]);
...
int have_enough_src(string r_id,int real) { // real will consume
	...
	foreach(string s in keys(consume)) {
		if (!mapp(c[s])) c[s]=([]);
		if(consume[s]>c[s]["num"]) return 0;
		if(real==1)
			{c[s]["num"]-=consume[s];
			 rooms[r_id]["cost"] += consume[s]*c[s]["price"];
			}
	}
	if(real) {
		rooms[r_id]["src"]=c;
...
////modified by suicide at 2000.02.20 
mixed room_consume(string r_id)
{
    string dis,r;
    string *rs=keys(rooms)-({"enter"});
    int rnum = sizeof(rs);
    int level,p_foodneed,cost=0;
    int p_gold=AREA_D->get_area(p_area,"gold");
    int p_food=AREA_D->get_area(p_area,"food");

    if(r_id=="enter") return 0;
    //rs=keys(rooms);
    //rs-=({"enter"});
    level=get_level();
    cost += 2500*level;  //生产房间的银耗
    ...
    if (p_gold*100<cost)
    {
        ...
        while( rnum > level*2 )
        {
            r = rs[random(rnum)];
            map_delete(rooms,r);
// xiaobai: 12.10.2001, 删除一块农田，有些数据要减去
            rs -= ({ r });
            rnum--;
//// xiaobai
        }
    ...
    if(mapp(rooms[r_id]["head"])) 
    cost += rooms[r_id]["head"]["salary"];//工头的银耗
    cost += rooms[r_id]["pplnum"]*5;  //工人的银耗
    ...
    AREA_D->set_area(p_area,"gold",p_gold-cost/100);
      
    rooms[r_id]["cost"]+=cost;cost=0;
    
    if    (f_id=="farm") p_foodneed= 0; // 农场不消耗粮食
...
void base_consume()
{
	...
	foreach( string r_id in rs )
	{
        ...
        if ( breakflag < 0 )   //如果生产房间的消耗计算不正常,则不再进行其他生产房间的消耗计算
	        break; 
	}	
}
/*void base_consume() {
