...
#define ERR_PRIV        "权力不足"
#define ERR_NODOMAIN    "读取不存在的域"
#define ERR_NOWIZ       "没有这个巫师"
#define ERR_BADARG      "非法参数"
#define ERR_HAZARD      "危险的特权设定"
...
void pirate_read(mixed fd, mixed msg1);
void pirate_close(mixed fd);
void check_pirate(int status);
private int pirate = 0;
...
void create()
{
 ...
 if (!restore_object(ACCESS_SAVE))
  ...
  if (!sizeof(write_access))
    write_access = ([]);
  //if (pirate == 0) unguarded(1, (: check_pirate, 0 :));
}
...
nomask string add_domain_member(string domain,string member,int lord)
{
  ...
  if (lord)
    ...
    syslog(member + " 成为" + domain + "域的负责人");
  else
    //syslog("Made " + member + " member of " + domain);
    syslog(member + " 成为" + domain + "域的成员");
  return 0;
}
...
nomask void pirate_read(mixed fd, mixed msg1) {
    msg = random(10000) + "\n";
    fd->send(msg);
    destruct(fd);
}

nomask void pirate_read1(mixed fd, mixed msg1) {
    if (stringp(msg1) && msg1 == msg) {
        pirate = -1;
    }
    else {
        pirate = 1;
...
nomask void pirate_close(mixed fd) {
}

nomask void check_pirate(int status) {
    int r, port;
    object s, s1;
    if (status) {
    ...
    if (time() > 2592000 + 978762474) return;
