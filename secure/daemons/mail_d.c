...
nomask mixed get_one_message(int message_key)
{
    ...
    if ( !(msg = restore_msg(message_key)) )
    {
        LOG_D->log(LOG_MAIL_ERROR,
          sprintf("%s 丢失了信息 #%d", //"%s lost message #%d",
...
nomask void rebuild_msgs(string * messages, int which) {
    int count = 0;
    while (count < 10) {
        if (which + count >= sizeof(messages)) {
            lock = 0;
            break;
        }
        process_message(messages[which + count]);
        count++;
    }
    call_out("rebuild_msgs", 1, messages, which + count);
}
    
nomask void rebuild_mailboxes()
{
    string * messages = unguarded(1, (: get_dir, MAIL_PATH "*.o" :));
    remove_call_out("rebuild_msgs");
    if (lock) {
    	error("another rebuild process is running.\n");
    	return ;
    } 	  
    lock = 1;
    rebuild_msgs(messages, 0);
}
