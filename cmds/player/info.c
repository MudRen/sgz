// info.c
// by fire on 7 July 1998, modified by row on Aug 1st, 2000
// this is used to 
...
void infocc(string a_id)
{
	...
	chars=CHAR_D->check_char_area("is_tmp",0,a_id);
	write("%^MAGENTA%^〓〓〓〓%^H_GREEN%^地区角色技能列表%^RESET%^%^MAGENTA%^〓〓〓〓〓〓%^RESET%^\n");
	if(sizeof(chars)) {
	write("    角色代号  角色姓名  武学修养  兵法修养  治国修养\n");
	foreach(string ch in chars)
	{
	who=find_body(ch);
			dis+=sprintf("%12s  %8s  %8s  %8s  %8s\n",
				ch,CHAR_D->get_char(ch,"name"),
...
+")" :));
   ...
   if(n_id==CHAR_D->get_char(p_id,"nation"))
      priorit=1;
   if((n_id==p_id)||(COUNTRY_D->get_country(n_id,"gs")==p_id))
      priorit=0;
   if(wizardp(this_body()))
      priorit=0;
...
private void main(string arg)
{
    ...
    if(!arg)
       arg="";
	//write("进了info.c main()函数. arg = " + arg + "\n");
    switch (arg)
    {
		case "me":
