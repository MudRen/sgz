// Updated by stefan on 10 Jan 1997 modified by fire on Mar,22, 1998
/* Do not remove the headers from this file! see /USAGE for more info. */
/*
 * Handle the ability of 'monsters' to 'target' other objects.
 *
 * One can rewrite this to, say, pick a random target instead of
concentrating
 * on one person, but be careful that query_target() must continually
return
...
object get_target() {
    ...
    if (!explicit) {
        ...
        explicit = 0;
#endif
    while (!target || target->query_ghost() || query_room() !=(target->query_room())) 
{
