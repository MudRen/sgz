...
//				);
		...
		// We assume since the condition daemon is loaded successfully, the//如果没有错误，它将调用自
                                                                                 //己的update_condition()函数.
...
//相当于cnd_d->update_condition(this_object(),conditions[cnd[i]]),见/kungfu/condition/下的各文件。
