...
//Emperor add for chat limit.
void set_chat_time(){
	chat_time = time();
}

int query_chat_time(){
	if ( !chat_time ) chat_time=time()-10;
	return chat_time;
}

void set_last_chat_msg(string msg){
	last_chat_msg = msg;
}

string query_last_chat_msg(){
	return last_chat_msg;
}
//Emperor add over.

/* emperor add for female char's gravidity */
void set_gravidity(){
	if(!is_gravidity) {
		is_gravidity=time();
		write("你胃中一酸，只想呕吐。\n");
...
void delete_gravidity(){is_gravidity=0;}
/* emperor add over */
...
this_user()->update_translations();
    ...
    else if ( is_new )
    {
      write("\n"
        ...
        /* these will be registered later */
        channel_list = ({ "gossip", "newbie","weather","rumor" });
        this_body()->query_shell_ob()->set_variable( "ansi" , 1);   
        this_user()->update_translations();
