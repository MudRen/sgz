#define HOME_SERVER "/sgdomain/home/home_server/" // 2001.10.30 added by yesh
...
mixed area_init4() {
	...
	foreach(string a in as) {
		...
		AREA_D->set_area(a,"zm",0); //2000.10.30 added by yesh
...
void init_char1() {
	string *as;
	int l;
	mapping home; //2001.10.30 added by yesh
	as=AREA_D->list_areas();
	l =sizeof(as);
...
//		CHAR_D->set_char(c,"area",as[random(l)]);
                ...
                */		    
// 2001.10.30 modified by yesh
                home=CHAR_D->get_char(c,"h");
	        if(!mapp(home)) CHAR_D->set_char(c,"area",as[random(l)]);
		else CHAR_D->set_char(c,"area",home["a"]);
}
