...
void do_recruit_soldier(string cfm)
{
	...
	zhili = CHAR_D->get_char(p_id,"skills")["sk_zhimou"];
	meili = CHAR_D->get_char(p_id,"skills")["sk_meili"];
	li = (wl*3 + zhili*2 + meili*5) / 10;
AREA_D->set_area(p_area,"recruit_soldier",(["year":p_year,"month":p_month]));
	soldiers = population* li / (8600*1.5);
	if(soldiers>10000)
	soldiers = 10000; 
	safe = safe - 30;
...
//      p_soldier=AREA_D->get_area(p_area,"soldier");
        ...
        p_train=(p_train*p_soldier+100*soldiers)/(p_soldier+soldiers);
        p_morale=(p_morale*p_soldier+150*soldiers)/(p_soldier+soldiers);
//      AREA_D->set_area(p_area,"soldier",p_soldier+soldiers);
 if( !troop["footman"] ) troop["footman"] = soldiers;
        else troop["footman"] = troop["footman"] + soldiers;
...
//    add_menu_item(toplevel, new_menu_item("派送商人", 	(: do_send_merchant :),"8"));
   ...
   add_menu_item(toplevel, new_menu_item("招募士兵", (:get_input_then_call,
		(: do_recruit_soldier :),"确认募兵吗？[y/n]" :) ,"8"));
