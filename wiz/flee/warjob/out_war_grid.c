...
mixed cell(int x, int y) {
  if( x < 0 ) return "XX";
  if( y < 0 ) return "XX";
  if( x > width )  return "XX";
  if( y > height ) return "XX";
...
int create_new_warmap(string map_id, mixed data)
{
    mixed tmp;
    ...
    if( !arrayp(data)||!(height=sizeof(data)) ) return 0;
    tmp = data[0];
    if( !arrayp(tmp)||!(width=sizeof(tmp)) ) return 0;
