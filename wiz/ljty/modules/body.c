...
int query_indoor_message()
{
  return indoor_message;
}

void set_indoor_message(int d)
{
  indoor_message = d;
}
//:FUNCTION set_plan
...
this_body()->start_age();
  ...
  p_wuli=this_body()->query_age();
  if(!p_wuli)
  {
     this_body()->init_hp();
  }
    p_wuli=this_body()->query_wuli_pure();
    if(!p_wuli)
    {
        new(SET_GIFT)->start_menu();    
    }
}
...
void move_or_destruct(object suggested_dest) {
    ...
    foreach (destination in ({ suggested_dest, VOID_ROOM, this_body()->query_start_location(), START, WIZARD_START  })) {
        ...
        if (destination && !err) {
            receive_private_msg(dested_env->short() + "被摧毁了，一阵时空转移将你送到" + destination->short() + "。\n");
            return;
        } else {
            if (destination)
...
/*         result = ({ new(class combat_result,
   ...
   commented by fire on March 14, 1998 */
