// suggest.c
// this is used for union meeting system
...
void discuss_fail(string n_id) {
	string msg;
	msg="看来关于"+m_suggest[n_id]["str"]+"的问题今天讨论不出结果，那就回头再说了。";
...
string check_suggest(string n_id){
	...
	if( !mapp(m_suggest[n_id]) ) return "目前没有任何案件。\n";
	if( (!m_suggest[n_id]["status"])||(m_suggest[n_id]["status"]=="over") )
		return "目前没有任何案件。\n";
	ret ="目前讨论案件："+m_suggest[n_id]["str"]+"\n";
