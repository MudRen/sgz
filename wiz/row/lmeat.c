...
string *desc = ({"这是一大块上好的%^B_WHITE%^%^H_RED%^猪肋%^RESET%^，肥薄肉厚，骨头都被剔干净了。\n"+
"不过一整块是没有人要的，需要用刀加工成大小均匀的%^B_WHITE%^%^H_RED%^猪肋排%^RESET%^来。\n(chop meat with tigudao)\n",
"这是一块上好的%^B_WHITE%^%^H_RED%^猪肋%^RESET%^，肥薄肉厚，骨头都被剔干净了。\n可以看得出这块肉被人切过。\n(chop meat with tigudao)\n",
...
void setup()
{
	set_id("meat", "%^B_WHITE%^%^H_RED%^大猪肋%^RESET%^","da zhulei");
	set_in_room_desc("一块%^B_WHITE%^%^H_RED%^大猪肋%^RESET%^(large meat)");
	status=25;
	set_unit("块");
	set_size(SMALL);
	set_can_drop(0);
...
void chop(object o)
{
	...
	this_body()->simple_action("$N举起%^H_CYAN%^剔骨刀%^RESET%^对准%^B_WHITE%^%^H_RED%^大猪肋%^RESET%^"+
	"猛地就是一下，可没想到力道使岔了，切下一块厚薄不一的肉片来。\n");
	tell_user(p_id,"唉，张屠户肯定不会要这样子的肋排的，扔了吧。\n\n");
	break;
	case 5:
	this_body()->start_busy(3,"你正忙着切肉呢。\n");
	this_body()->simple_action("$N举刀切了下去，糟了，差点儿割到自己的手指，$N一慌，不小心把刚切下的肉碰掉在地上的臭水塘里。\n");
	tell_user(p_id,"完了，张屠户卖肉的声誉极好，他是不会要这种脏肉的，还是扔了吧。\n\n");
	break;
	default:
	this_body()->start_busy(3,"你正忙着切肉呢。\n");
	this_body()->simple_action( ({"$N用并不熟练的手法操着%^H_CYAN%^剔骨刀%^RESET%^，小心翼翼地从%^B_WHITE%^%^H_RED%^大猪肋%^RESET%^上切下一块肉片。\n"+
	"嗯，厚薄均匀，肥瘦适中，是一块合格的%^B_WHITE%^%^H_RED%^猪肋排%^RESET%^。\n\n",
	"$N左手将%^B_WHITE%^%^H_RED%^大猪肋%^RESET%^按在案台上，右手攥紧%^H_CYAN%^剔骨刀%^RESET%^，慢慢地切下一片肉来。\n"+
	"嗯，厚薄均匀，肥瘦适中，是一块合格的%^B_WHITE%^%^H_RED%^猪肋排%^RESET%^。\n\n",
	"$N一边看着张屠户的手势，一面学着从%^B_WHITE%^%^H_RED%^大猪肋%^RESET%^上切下一片肉来，还挺象那么一回事。\n"+
	"嗯，厚薄均匀，肥瘦适中，是一块合格的%^B_WHITE%^%^H_RED%^猪肋排%^RESET%^。\n\n"}) );
	ob=new(__DIR__"pork");
	ob->move(this_body());
	break;
