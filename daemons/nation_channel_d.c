// nation_channel_d.c
// this is used for national channel 
// by fire on April 1999
inherit __DIR__+"nation_channel_d/suggest";
...
mixed query_list(string p_nation) {
	...
	list=filter_array(list,(:CHAR_D->get_char($1,"nation")==$(p_nation):));
...
void deliever_tell(string p_id,string n_id,string msg) {
	...
	msg="%^H_CYAN%^【"+COUNTRY_D->get_country(n_id,"name")+"】"+teller+msg;
	if(msg[<1]!='\n')
		msg+="\n";
...
mixed deliever_enter(string p_id, string msg) {
	...
	if(msg[0]=='+') // wizard regist
	{
		...
		if(!wizardp(o)) return "只有巫师才可以窃听他国情报。\n";
		n_id=msg[1..<1];
		if (n_id == "all") {
