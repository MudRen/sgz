				 ...
				 string target_user, mixed * message)
{
	...
	/*tell_user("huaer",sprintf("准备给router发送信息:\n"));
	tell_user("huaer",sprintf("type = %s\n",type));
	tell_user("huaer",sprintf("mud_name() = %s\n",mud_name()));
	tell_user("huaer",sprintf("orig_user = %s\n",orig_user));
	tell_user("huaer",sprintf("target_mud = %s\n",target_mud));
	tell_user("huaer",sprintf("target_user = %s\n",target_user));
...
{
    send_message(type, "*wpr", 0, message);
}
...
private nomask void reconnect()
{
	...
	send_to_router("startup-req-3",
		       ({ password,	//以前是password2139547924
			      query_mudlist_id(),
			      query_chanlist_id(),
			      __PORT__,
			      PORT_I3_TCP_OOB,
...
void create()
{
    ...
    mudlist_reset_entries();			//为mudlist初始化接口

    reconn_func = (: reconnect :);		//发起连接

    oob_startup();						//oob带外数据连接服务初始化
    chan_startup();						//频道初始化
    file_startup();						//文件传送初始化
    mail_startup();						//邮件服务初始化
