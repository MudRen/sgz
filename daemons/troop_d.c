...
void troop_heart()
{
	...
	foreach(int p_id in keys)
	{
		...
		int is_auto=1;
		int ret=0;
		//tell_user("huaer",sprintf("在troop_heart()中troops的键值p_id = %d\n",p_id));
		ret=illegal_troop(p_id);
		//tell_user("huaer",sprintf("illegal_troop()的返回值是 %d\n",ret));
		if(ret)
		{	
...
{
	//tell_user("huaer",sprintf("troop_d中 p_id = %d\n",p_id));
        if(!troops[p_id])
        {
...
int put_troop(int p_id,string r_id)
{
	...
	//printf("在troop_d.c/put_troop()函数中，参数p_id=%d, r_id=%s\n",p_id,r_id);
   ...
   string p_type,int p_num,int p_mol,int p_tran,int p_eng)
{
	...
	{
	//remed by suicide in 2001.8.8 for have dec population to commando before def_arrange
	//So need dec here again, or will cause population lost
/*		if( p_type == "commando" )
			AREA_D->set_area(s_area,"population",
                                AREA_D->get_area(s_area,"population")-p_num);
		else*/
...
int illegal_troop(int t_id)
{
	...
	//tell_user("huaer",sprintf("find_troop()返回对象是%O\n",o));
	env=environment(o);
...
string get_troop_symbol( int par_nTrpId )
{
    ...
    string tr_strLeaderName = CHAR_D->get_char( tr_strLeaderId, "name" );
    
    tr_strLeaderName = colour_truncate( tr_strLeaderName, 2 );
    
    return tr_strLeaderName[0..1];

}  // end get_troop_symbol
