...
*/
string fix_color(string str,string channel_name) {
   string o_color=upper_case(channel_name);
   o_color="%^RESET%^"+"%^"+o_color+"_CHANNEL%^";
   return replace_string( str, "%^RESET%^", o_color );
}
...
nomask void deliver_channel(string channel_name, string str)
{
    ...
    if (channel_name == "imud_imud_sgz") {
    	...
    	if (sscanf(str, "%s@%s：%s", sender, mud, msg) == 3) {
    	    cname = mud;
    	    str = sender + "：" + msg;
    	}
    ...
    str=fix_color(str,channel_name);
    
    deliver_string(channel_name,
             sprintf(CHANNEL_FORMAT, upper_case(channel_name),
             cname, str));
}
                 ...
                 mixed data)
{
	...
	//tell_user("huaer",sprintf("进了channel_d.c/deliver_data(),ci = %O\n",ci));
    if ( !ci || sizeof(ci->listeners) == 0 )
    return;
...
deliver_notice("announce", sprintf("谣言：%s。", tmp));
}
}
...
nomask void deliver_soul(string channel_name, mixed * soul)
{
    ...
    soul = ({ soul[0] }) +
    ({ map_array(soul[1],
             (: sprintf(CHANNEL_FORMAT, upper_case($(channel_name)), query_channel_name($(channel_name)), $1[0..<2]) :))
       });
...
nomask void deliver_notice(string channel_name,
               string message)
{
    deliver_channel(channel_name, "(" + message + ")");
}
...
nomask object * query_listeners(string channel_name)
{
    if ( check_previous_privilege(1) ||
     ...
     previous_object() == find_object(DAY_D))
    {
    class channel_info ci = info[channel_name];
