...
nomask void remove_chanlist(string channel_name) {
				      ...
				      string targ_user, mixed * message)
{
	...
	return_error(orig_mud, orig_user, "unk-user",
		     sprintf("没有「%s」这个玩家", targ_user));
    }
    else
    {
	tell(p, sprintf("「%s@%s」的收听者有：",
				    message[0], orig_mud) +
			    implode(message[1], ", ") + "。\n");
    }
}
...
protected nomask void rcv_channel_m(string orig_mud, string orig_user,
				 string targ_user, mixed * message)
{
	/*tell_user("huaer",sprintf("进了/imud/channel.c/rcv_channel_m()\n"));
				 ...
				 string targ_user, mixed * message)
{
    ...
    */
    if (sscanf(message[2], "%s(from $N)", msg) != 1) msg = message[2];
    CHANNEL_D->deliver_channel("imud_" + message[0],
     	sprintf("%s+%s", orig_mud, msg ));
}
